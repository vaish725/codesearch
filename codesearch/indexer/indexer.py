...
class Indexer:
    ...
    def index_repository(
        ...
        try:
            ...
            for file_path in scanner.scan():
                ...
                try:
                    ...
                    if result:
                        ...
                        cursor = conn.execute("SELECT file_id FROM files WHERE path = ?", (rel_path,))
                    ...
                    for symbol in symbols:
                        conn.execute(
                            ...
                            VALUES (?, ?, ?, ?, ?, ?)
                            """,
                            (file_id, symbol.name, symbol.kind, symbol.start_line, symbol.end_line, symbol.signature)
                        )
                    ...
                    if batch_count >= DB_BATCH_SIZE:
                        ...
                        logger.info(f"Indexed {indexed_total} files so far ({stats['files_new']} new, {stats['files_updated']} updated)...")
                        batch_count = 0
            ...
            # Log summary
            if force:
                logger.info(f"Full indexing complete: {stats['files_indexed']} files indexed in {duration:.2f}s")
            else:
                logger.info(
