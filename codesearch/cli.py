...
def command_index(args):
    ...
    try:
        stats = indexer.index_repository(repo_path, force=args.force if hasattr(args, 'force') else False)
        ...
        print(f"   Total in DB: {db_stats['total_files']} files, {db_stats['total_symbols']} symbols, {total_size_mb:.2f} MB")
...
def command_stats(args):
    ...
    try:
        ...
        if stats['symbols_by_kind']:
            print(f"\n   By kind:")
            for kind, count in sorted(stats['symbols_by_kind'].items(), key=lambda x: x[1], reverse=True):
                print(f"      {kind}: {count}")
...
def main():
    ...
    query_parser.add_argument("--context", type=int, default=0, help="Number of context lines to show around matches (0=disabled)")
